...
public class CustomerController {
    ...
    public ResponseEntity<CustomerDetailsDTO> fetchCustomerDetails(@RequestHeader("banking-microservices-correlation-id") String correlationId,
                                                                   @RequestParam
                                                                   @Pattern(regexp = "(^$|[0-9]{10})", message = "Account number should be 10 digits")
                                                                   String mobileNumber) {
        logger.debug("banking-microservices-correlation-id found: {}", correlationId);
